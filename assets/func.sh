...
function usercheck {
    if [ "$USER" != "pi" ]; then
        echo 'Run this script as the "pi" user.'
        exit 1
    fi
}
...
system. If you want to perform these steps now, then press the "y" key.
...
function net_error {
    ...
    echo
    echo "If you still feel it's a bug, then please create an issue here:"
...
'; printf "\e[0m"; sleep 2
    ...
    sudo apt install -y automake gobjc libudev-dev xa65 build-essential byacc texi2html flex \
                        ...
                        libpcap0.8-dev libvte-dev libasound2-dev linux-headers-$(uname -r)
